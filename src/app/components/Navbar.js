...
export default function Navbar() {
  ...
  useEffect(() => {
    if (isHomePage) {
      setShowNorya(false);
      const timer = setTimeout(() => setShowNorya(true), 3200);
  ...
  return (
    <nav
      className={`w-full z-50 text-white bg-transparent pt-4 fixed top-0 left-0 transition-opacity duration-500 opacity-100`}
    >
      <div className="max-w-9xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6 h-full m-0 flex-nowrap">
            ...
            <Link href="/products/cart" className="hover:bg-yellow-400 hover:text-black rounded-full px-4 py-1 group transition-all">
              <img className="w-5" src="/shoppingCartIconWhite.png" alt="Cart" />
            </Link>
            <Link href="/profile" className="group hover:bg-yellow-400 rounded-full px-4 py-1 transition-all">
