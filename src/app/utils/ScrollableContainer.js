...
export default function Navbar() {
  ...
  const [atTopCount, setAtTopCount] = useState(0);
  const [atBottomCount, setAtBottomCount] = useState(0);
  const [atTop, setAtTop] = useState(true);

  const lastScrollY = useRef(0);

  // Update sizes for responsive logo
  useEffect(() => {
    const updateSizes = () => {
  ...
  // Show NORYA text on homepage after delay
  useEffect(() => {
    if (isHomePage) {
      setShowNorya(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Detect top
      if (scrollY === 0 && lastScrollY.current !== 0) {
      ...
      // Detect bottom
      if (scrollY + clientHeight >= scrollHeight - 1 && lastScrollY.current + clientHeight < scrollHeight - 1) {
        console.log("Reached bottom");
        setAtBottomCount(c => c + 1);
      }
  ...
  return (
    <nav
      className={`w-full z-50 text-white bg-transparent pt-4 fixed top-0 left-0 transition-all duration-500 ${
    ...
    >
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6 h-full m-0 flex-nowrap">
            {[{ href: "/sellers", label: "Selgere" },
              ...
              { href: "#", label: "Favoritter", extra: "pt-2" }].map((item) => (
      ...
      {/* Top/Bottom counters display */}
      <div className="absolute bottom-0 right-0 text-white text-sm pr-4 pb-1">
        Top reached: {atTopCount} | Bottom reached: {atBottomCount}
      </div>
      ...
      >
        ...
        >
          {[{ href: "/sellers", label: "Selgere" },
            ...
            { href: "/profile", label: "Profil" }].map((item) => (
            <Link
              key={item.label}
              href={item.href}
